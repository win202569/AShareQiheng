...
- Observation 的公开证明方法/属性查找/继承替换不能绕过原身份检查；原 source 模块初始化时提供私有 `_range_observation_authority()`，只返回原 observation type/current verifier/wire reader 的不可变 tuple，供 W1 提前封存，不导出 mint 或注册入口。W1 仍须使用本库观察并核对同库/根。
