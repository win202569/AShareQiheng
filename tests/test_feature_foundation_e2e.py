...
class HermeticDeepHarness:
    ...
    def run(self) -> ReplayResult:
        ...
        bundles = tuple(
            ...
            for row in sorted(
                rows, key=lambda item: (item["security_id"], item["input_hash"])
            )
        )
...
class FeatureFoundationEndToEndTests(unittest.TestCase):
    def test_three_security_replay_is_traceable_deterministic_and_nonformal(self):
        ...
        self.assertEqual(
            {bundle.security_id for bundle in first.bundles},
            {"SH600001", "SZ000001", "SH600002"},
        )
    ...
    def test_unclassified_and_balance_mismatch_are_blocked(self):
        ...
        self.assertTrue(
            all(
                ...
                and "industry_template_unclassified" in bundle.blockers
                for bundle in unclassified_result.bundles
            )
        )
        self.assertTrue(
            any(
                bundle.financial_status == "blocked"
                and "balance_equation_mismatch" in bundle.blockers
                for bundle in mismatch_result.bundles
            )
        )
